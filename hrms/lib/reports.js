...
class ReportService {
  ...
  async generateAttendancePDF(attendance, report) {
    ...
    // Add table
    this.addTableToPDF(doc, headers, data, { startY: 40 });
  ...
  async generateAttendanceExcel(attendance, report) {
    ...
    const filePath = `/reports/${fileName}`;
    await workbook.xlsx.writeFile(`public${filePath}`);
    
    const stats = await this.getFileStats(`public${filePath}`);
  ...
  async generatePayrollPDF(payrolls, report) {
    ...
    doc.text(`Total Net Salary: $${totalNetSalary.toFixed(2)}`, 14, 40);
    doc.text(`Number of Employees: ${payrolls.length}`, 14, 50);
    ...
    // Add table
    this.addTableToPDF(doc, headers, data, { startY: 60 });
  ...
  async generateEmployeePDF(employees, report) {
    ...
    // Add table
    this.addTableToPDF(doc, headers, data, { startY: 50 });
  ...
  addTableToPDF(doc, headers, data, options = {}) {
    const { startY = 40 } = options;
    ...
    headers.forEach((header, index) => {
      doc.text(header, 14 + (index * columnWidth), y);
    });
    ...
    data.forEach(row => {
      ...
      row.forEach((cell, index) => {
        doc.text(cell.toString(), 14 + (index * columnWidth), y);
      });
